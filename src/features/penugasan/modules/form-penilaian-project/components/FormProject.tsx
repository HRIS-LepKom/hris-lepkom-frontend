...
const schema = z.object({
  ...
  kontribusi: z.number().min(0).max(100),
  presentasi: z.number().min(0).max(100),
  motivasi: z.number().min(0).max(100),
  interpersonal: z.number().min(0).max(100),
  integritas: z.number().min(0).max(100),
  potensi: z.number().min(0).max(100),
...
const FormProject = () => {
  ...
  return (
    <ContentLayout>
      ...
      {/* Two-column layout: Kolom 1 (70% scrollable) & Kolom 2 (30% sticky) */}
